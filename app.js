...
const translations = {
  en: {
    ...
    flatRoof: 'Flat Roof', gableRoof: 'Gable Roof', mansard: 'Mansard', roomsLayout: 'Rooms & Layout',
    bedrooms: 'Bedrooms', bathrooms: 'Bathrooms', livingRoom: 'Living Room', dining: 'Dining',
    kitchen: 'Kitchen', store: 'Store', study: 'Study', garage: 'Garage',
  ...
  ha: {
    ...
    readyToGenerate: 'Shirye don samarwa', view3D: 'Kallonin 3D', frontFacade: 'Gaban Gini',
    floorPlan: 'Tsarin Bene', interiorView: 'Ciki na Gini', techSpecs: 'Bayanin Fasaha',
  ...
  yo: {
    ...
    readyToGenerate: 'Ṣetán láti ṣẹ̀dá', view3D: 'Ìwòran 3D', frontFacade: 'Iwájú Ilé',
    floorPlan: 'Ètò Ilẹ̀ Ilé', interiorView: 'Inú Ilé', techSpecs: 'Àlàyé Ìmọ̀-ẹ̀rọ',
  ...
  ig: {
    ...
    readyToGenerate: 'Dị njikere imepụta', view3D: 'Ọhụụ 3D', frontFacade: 'Ihu Ulo',
    floorPlan: 'Atụmatụ Ala Ulo', interiorView: 'ime Ulo', techSpecs: 'Nkọwa Teknụzụ',
  ...
  fr: {
    ...
    frontFacade: 'Façade Avant', floorPlan: 'Plan de Masse', interiorView: 'Vue Intérieure',
    techSpecs: 'Spécifications Techniques', generatingExterior: 'Génération extérieure...',
    renderingFacade: 'Rendu de la façade...', drawingFloorPlan: 'Dessin du plan...',
    visualizingInterior: 'Visualisation intérieure...', compilingSpecs: 'Compilation des specs...',
    ph3D: 'La vue 3D apparaîtra ici', phFacade: 'La façade apparaîtra ici',
    phFloorPlan: 'Le plan apparaîtra ici', phInterior: 'L\'intérieur apparaîtra ici',
  ...
  ar: {
    ...
    frontFacade: 'الواجهة الأمامية', floorPlan: 'مخطط الطابق', interiorView: 'المنظر الداخلي',
    techSpecs: 'المواصفات التقنية', generatingExterior: 'جاري إنشاء الخارج...',
    renderingFacade: 'جاري رسم الواجهة...', drawingFloorPlan: 'جاري رسم المخطط...',
    visualizingInterior: 'جاري تصور الداخل...', compilingSpecs: 'جاري تجميع المواصفات...',
    ph3D: 'سيظهر العرض ثلاثي الأبعاد هنا', phFacade: 'ستظهر الواجهة هنا',
    phFloorPlan: 'سيظهر المخطط هنا', phInterior: 'سيظهر الداخل هنا',
...
function isPro() {
  return getUserPlan() === 'pro' || getUserPlan() === 'studio';
}
...
function updateGenerationCounter() {
  if (isPro()) {
    ...
    if (counter) counter.textContent = '∞ Unlimited generations';
    return;
  }
  const count     = getGenerationCount();
  ...
  if (counter) {
    ...
    counter.style.color = remaining === 0 ? '#F09595' : remaining === 1 ? '#E2A057' : '#8A8880';
  }
}
...
function showUpgradeModal(reason) {
  ...
  if (reason === 'generation') {
    ...
    subtitle.textContent = 'You have used all 3 free generations for today. Upgrade to Pro for unlimited designs every day!';
  } else if (reason === 'save') {
    title.textContent    = '💾 Pro Feature';
    subtitle.textContent = 'Saving projects is a Pro feature. Upgrade to save unlimited designs and access them anytime!';
  } else if (reason === 'specs') {
    title.textContent    = '📋 Pro Feature';
    subtitle.textContent = 'Technical specifications are a Pro feature. Upgrade to get full AI-generated material lists and building specs!';
  } else if (reason === 'language') {
    title.textContent    = '🌍 Pro Feature';
    subtitle.textContent = 'Language switching is a Pro feature. Upgrade to access Hausa, Yoruba, Igbo, French and Arabic!';
  } else if (reason === 'settings') {
    title.textContent    = '⚙️ Pro Feature';
    subtitle.textContent = 'Advanced settings are a Pro feature. Upgrade to access accent colors, account management and more!';
  }
...
function buildSummary(beds, baths, type, plotW, plotL, seed) {
  ...
  grid.innerHTML = `
    ...
    <div class="summary-item"><div class="summary-val">${isPro() ? '∞' : FREE_DAILY_LIMIT - getGenerationCount()}</div><div class="summary-key">${isPro() ? 'Pro Plan' : 'Left Today'}</div></div>
  `;
  ...
  return svgCode.replace(
    ...
    `<rect x="0" y="0" width="100%" height="100%" fill="transparent"/>
     <text x="50%" y="50%" text-anchor="middle" dominant-baseline="middle"
       ...
       transform="rotate(-30, 260, 160)" font-weight="bold">
       MY ArchGen FREE
     </text>
     </svg>`
  );
...
function generate3DSVG(config) {
  ...
  const floorH = 80; const buildingW = 200; const buildingH = floors * floorH;
  const roofH = roof === 'Flat Roof' ? 10 : 50;
  const cx = 260; const cy = 80; const depth = 60;
  const windows = [];
...
function generateFacadeSVG(config) {
  ...
  const roofColor = '#8B4513'; const trimColor = '#8B6914';
  const floors = type === 'Duplex' ? 2 : 1; const floorH = 90;
  const buildingW = 320; const buildingH = floors * floorH;
  const startX = 100; const startY = 240 - buildingH;
...
async function generate() {
  ...
  // Technical specs — Pro only
  if (isPro()) {
    try {
      const specResult = await callAI(`Generate technical specifications for: ${baseDesign}. Include foundation type for ${soil} soil, structural system, wall construction, roofing material, floor area in sq ft, columns, plumbing, electrical, Nigerian climate materials, and 3 zoning notes for ${zoning}.`);
      showResult(4, specResult);
    } catch (error) {
      showResult(4, 'Technical specs unavailable — check your API key.');
    }
  } else {
    // Show locked specs for free users
    ...
    res4.innerHTML = `
      <div style="text-align:center;padding:30px 20px;">
        ...
        <div style="font-size:12px;color:var(--text-muted);margin-bottom:16px;line-height:1.6;">Technical specifications are available on the Pro plan. Upgrade to get full AI-generated material lists, foundation details and building specs.</div>
        <button onclick="showUpgradeModal('specs')" style="background:linear-gradient(135deg,#B8860B,#C9A84C);border:none;color:#0F0E0C;font-family:'Syne',sans-serif;font-weight:700;font-size:12px;padding:10px 20px;border-radius:6px;cursor:pointer;letter-spacing:0.06em;text-transform:uppercase;">
          ⚡ Upgrade to Pro
        </button>
      </div>
    `;
...
console.log('MY ArchGen — Full Monetization Active!');
